...
namespace Detection
{
    void YoloX::SetParam(const YoloParam& param)
    {
        m_param = param;
    }

    void YoloX::GetParam(YoloParam& param) const
    {
        param = m_param;
    }
    ...
    void YoloX::Run()
    {
        ...
        while(true)
        {
            ...
            for(auto& obj : boxes)
            {
                ...
                auto name    = DettectLabels[obj.class_label];
    ...
    void YoloX::Run(const cv::Mat& img)
    {
        if(img.empty())
        {
            return;
        ...
        //提交图像
        std::shared_future<Yolo::BoxArray> boxes_list = m_pEngine->commit(img);
    ...
    void YoloX::Run(const std::vector<cv::Mat>& vecImg)
    {
        if(vecImg.empty())
        {
            return;
    ...
    bool YoloX::CreateEngine()
    {
        m_pEngine = Yolo::create_infer(m_param.trtModelFile, Yolo::Type::X, 0, m_param.score, m_param.nmsThreshold);
    ...
    void YoloX::Compile()
    {
        ...
        auto int8process = [=](int current, int count, const vector<string>& files, shared_ptr<TRT::Tensor>& tensor)
        {
            ...
            for(int i = 0; i < files.size(); ++i)
            {
                ...
                Yolo::image_to_tensor(image, tensor, Yolo::Type::X, i);
            }
        };
    ...
    void YoloX::SetImage(const cv::Mat& image)
    {
        m_image = image.clone();
    }
}
